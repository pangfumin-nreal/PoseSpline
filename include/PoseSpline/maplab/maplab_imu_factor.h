...
class IntegrationBase{
    ...
    public:
        ...
        IMUFactor(
            ...
            ImuParam imu_param;
            imu_param.ACC_N = acc_noise_sigma;
            imu_param.ACC_W = acc_bias_sigma;
            imu_param.GYR_N = gyro_noise_sigma;
            imu_param.GYR_W = gyro_bias_sigma;

            Eigen::Vector3d accel = imu_data.col(0).head<3>();
            Eigen::Vector3d gyro = imu_data.col(0).tail<3>();

            pre_integration_ = std::make_shared<maplab::IntegrationBase>(
                accel, gyro, ba0, bg0, imu_param);
        ...
        bool EvaluateWithMinimalJacobians(double const *const *parameters,
            ...
            Eigen::Vector3d G{0.0, 0.0, gravity_magnitude_};
            if (jacobians)
            {
